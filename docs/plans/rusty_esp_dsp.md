...
> D2 ◐ software half on the host 2026-09-02 (the seam, `-esp` with delegating `PieS3`/`PieP4`, the twin gate, the Xtensa compile check; board half ☐) · D3 ☐ (board, software half first) · D4 ☐ (board).
...
| 2026-09-02 | And it was: the magic-number round (`+1.5·2^23 −1.5·2^23`) replaced `rintf`, bit-identical over all 2^32 inputs (exhaustive test, `--ignored`, 16 s), 2.8× per sample on the host, counter 16 000 libm calls → 0. Rule kept: an exhaustive oracle over the whole input space beats any corpus when the space is 2^32. |
...
| 2026-09-02 | `yuyv_to_rgb888`'s chroma-sharing restructure was measured (pinned, ABBA, two pairs) and reverted: the compiler already shares it; the kernel's cost is its byte layout and clamps, which is the S3 twin's business. The scalar kernel stays as written. Ledger has the runs. |
| 2026-09-02 | The seam is three traits (`PixelKernels`, `SampleKernels`, `BlockKernels`) with `Scalar` implementing all three, not one trait per kernel and not a `cfg`-switched function table: a function package names the family it needs, a firmware picks one type in one place, and the gate (`twin_matches_scalar`) can hold a whole type to the oracle at once. `-esp` ships before any twin, delegating, so the crate that will carry the first fenced `unsafe` already builds for `xtensa-esp32s3-none-elf` and is already in the gate. The S3 check is local only: CI runners have no esp toolchain, and installing one there is not worth a job until a twin exists. |
