# Generic paired A/B for two DIFFERENT command lines (a scalar arm vs a twin,
# ours vs a reference), pinned, High priority, measured by CPU time: affinity
# restricts us but does not reserve the core, so elapsed wall counts
# descheduled time; CPU time does not accrue off-core and is ~5x tighter
# under a foreign load. Ported from rs_h264's bench/pinvs.ps1 (the family's
# one compliant timing harness; codec-measurement 13) so this repo does not
# grow a second, drifting copy of the discipline.
#
#   pinvs.ps1 -AExe scalar.exe -AArgs @('...') -BExe twin.exe -BArgs @('...') -Pairs 15
#
# Reports the median CPU-time ratio A/B and the paired win count with its z.
# Because the two arms may be DIFFERENT PROGRAMS, the ratio is a throughput
# comparison, not a regression check: state the work-identity check (the
# `probe::Work` counters both arms print) separately, or the number is void.
#
# ALSO prints wall + cores-busy (= cpu/wall) every pair. A 1-core pin with
# cores-busy >> 1 is nested threads thrashing, not a 1T number.
...
"method: pinned mask 4, High, CPU time, {0} pairs ABBA; work parity is the caller's check" -f $n
